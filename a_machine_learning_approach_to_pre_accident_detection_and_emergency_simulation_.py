...
st.title("Pre-Accident Detection App")
st.write(data.head())
...
st.write("Predicted Severity:", colorize(predicted_label))
st.write("Actual Severity:", colorize(actual_label))
...
for i in range(10):
    ...
    st.write(f"\n Sample {i+1}")
    st.write("Predicted Severity:", termcolor.colored(predicted_label, "green" if predicted_label == "Slight Injury" else "yellow" if predicted_label == "Serious Injury" else "red"))
    st.write("Actual Severity:", termcolor.colored(actual_label, "green" if actual_label == "Slight Injury" else "yellow" if actual_label == "Serious Injury" else "red"))

    if predicted_label in ["Serious Injury", "Fatal Injury"]:
        st.write(" Emergency Trigger Activated")
        driver_info = {
            "Age": sample["Age_band_of_driver"].values[0],
            "Sex": sample["Sex_of_driver"].values[0],
            "Vehicle": sample["Type_of_vehicle"].values[0],
        ...
        st.write(" No emergency triggered.")
...
st.write("🚗 Predicted Accident Severity:", termcolor.colored(
    predicted_label,
...
if emotional_risk >= 5:
    st.write("🔊 Emotion Alert: Driver stress detected!")
    st.write("🎶 Activating calming music...")
    st.write("🚨 Sending pre-alert to emergency services...")
else:
    st.write("✅ Driver emotional state stable.")
...
if emotional_risk >= 5:
    ...
    st.write("🚑 Nearest Ambulance Notified")
    st.write("🚓 Traffic Control Updated")
    st.write("🛰️ Location Broadcasted to Emergency Grid\n")
